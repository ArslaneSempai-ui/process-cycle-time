...
export const DOCUMENTED_PATH: Activity[] = [
...
export type Event = {
  caseId: string;
  activity: Activity;
  /** Minutes since the start of the log. */
  at: number;
  /** How long somebody was actually working on it. Zero for instantaneous events. */
  touchMinutes: number;
  actor: string;
};
...
function lognormalish(r: () => number, median: number, spread: number): number {
  const u = r() + r() + r() - 1.5;
 ...
 */
export const TOUCH_MINUTES: Record<Activity, number> = {
  received: 2,
  triaged: 12,
...
export type Config = {
  cases: number;
  /** Probability a case needs information, sending it backwards. */
  reworkChance: number;
  /** Probability a case that already came back comes back again. */
  reworkAgainChance: number;
  /** Median wait between two activities, in minutes. */
  medianWaitMinutes: number;
  /** Median wait while an information request sits with the customer. */
  medianCustomerWaitMinutes: number;
  /** Share of cases that end in rejection rather than approval. */
  rejectShare: number;
  /**
   * Share of cases that arrive already triaged, through a channel that does it upstream.
   *
   * A step being skipped legitimately is the most common source of non-conformance, and
   * the one a conformance report most often flags as a violation when it is a second,
   * undocumented, perfectly fine route.
   */
  preTriagedShare: number;
  /** Share of cases that go to a second pair of eyes. */
  escalateShare: number;
  /** Chance the customer has to be chased on an outstanding request. */
  chaseChance: number;
  seed: number;
};
...
export function generate(c: Config = CONFIG): Event[] {
  const r = draw(c.seed);
  const events: Event[] = [];
  ...
  for (let i = 0; i < c.cases; i++) {
    ...
    const emit = (activity: Activity, wait: number) => {
      clock += wait;
    ...
    if (r() >= c.preTriagedShare) emit("triaged", lognormalish(r, c.medianWaitMinutes, 0.8));
    ...
    while (r() < chance && passes < 4) {
      ...
      if (r() < c.chaseChance) emit("chased", lognormalish(r, c.medianCustomerWaitMinutes, 0.7));
    ...
    if (r() < c.escalateShare) {
      ...
      if (r() < 0.45) emit("risk assessed", lognormalish(r, c.medianWaitMinutes * 1.5, 0.8));
    }
...
export function byCase(events: Event[]): Map<string, Event[]> {
  const cases = new Map<string, Event[]>();
  for (const e of events) {
    const list = cases.get(e.caseId);
    if (list) list.push(e);
    else cases.set(e.caseId, [e]);
  }
  for (const list of cases.values()) list.sort((a, b) => a.at - b.at);
  return cases;
}

export const MINUTES_PER_WORKING_DAY = 8 * 60;
export const days = (minutes: number) => minutes / MINUTES_PER_WORKING_DAY;
