 ...
 */
import { generate, byCase, DOCUMENTED_PATH } from "./events.js";
...
export const short = (path) => path.map((a) => ABBREV[a]).join(" ");
export function variants(events = generate()) {
...
export function conformance(events = generate()) {
 ...
 */
export function activityGaps(events = generate()) {
...
if (isMain(import.meta)) {
    ...
    for (const x of v.slice(0, 8)) {
        console.log(`  ${String(x.cases).padStart(5)}  ${pc(x.share).padStart(7)}   ${String(x.reworkPasses).padStart(6)}   ` +
            short(x.path) + (x.documented ? "   ← documented" : ""));
    }
    if (v.length > 8)
        console.log(`  ${String(v.length - 8).padStart(5)} more routes below this`);
